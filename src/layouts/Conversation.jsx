...
const handleSendMessage = (e) => {
  ...
  set(push(ref(db, "messages/")), newMsg)
    .then(() => {
      ...
      const validMembers = membersId
        .filter((member) => {
          const isValid = member && member !== user?.uid && member.trim() !== "";
          return isValid;
        });


      validMembers.map((memberId) => {
        return set(push(ref(db, "messagenotification/")), {
          projectId: id,
          reciverId: memberId,
        });
      });
    ...
    .then(() => {
      if (currentProject?.adminId && currentProject.adminId !== user?.uid) {
        return set(push(ref(db, "messagenotification/")), {
          projectId: id,
          reciverId: currentProject.adminId,
        });
      }
    })
    .catch((error) => {
  ...
  return (
    <div onClick={removeMsgNotif} className="flex-1 flex flex-col">
      {/* Chat Header */}
